    ...
    <div style="margin-right:50px">
            ...
            <li><a href = "dashboard.php" style="color:white;" role="button "><b>DashBoard</b></a></li>
    ...
    <form class="form-horizontal" action="Usersignup.php" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" name="prof" id="description">
        </div>
      </div>
         <div class="form-group">
  ...
  if(!empty($fname)&&!empty($lname)&&!empty($prof)&&!empty($age)&&!empty($nid)&&!empty($uname)&&!empty($password)&&!empty($area)&&!empty($number)&&!empty($email))
    {
  $target_dir = "uploads/";
  ...
  if ($uploadOk == 0) {
      ...
      if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file1)) {
        ...
        $query="INSERT INTO customerregistration(FirstName, LastName, Profession, Age, NID, Username, password, Area,review,image,phonenumber,email,date) 
        VALUES('$fname','$lname', '$prof', '$age','$nid','$uname','$password', '$area','10','$target_file1','$number','$email','$now_date');";
